...
function createUI(e, reportUrlVal) {
  ...
  var generateReport = CardService.newTextButton()
...
function generateReport(e) {
  ...
  if (cloudInstance) {
    if (reportType == 'mostExecutedScript') {
      ...
      reportTitle = 'Report for Most Executed Script of ' + 
            projTitle + ' for ' + timestampHeader;
    } else if (reportType == 'mostActiveUsers') {
      urlEnd = 'mostActiveUsers';
      reportTitle = 'Report for Most Active Users of ' + 
            projTitle + ' for ' + timestampHeader;
    } else {
      urlEnd = 'emailOfOwnerOfScript';
      reportTitle = 'Report for Owners of Apps Script of ' + 
            projTitle;
    }
    var token = 
  ...
  if (reportType == 'mostExecutedScript') {
    ...
    var reportUrl = generateReportForMostExecutedScript(
      mostExecutedScript, 
      'Report for Most Executed Script of ' + 
          projTitle + ' for ' + timestampHeader, 
      limitResponse);
  } else if (reportType == 'mostActiveUsers') {
    var mostActiveUser = getMostActiveUser(
      fromTime, 
      projectFilter, 
      input.projectId);
    var reportUrl = generateReportForMostActiveUsers(
      mostActiveUser, 
      'Report for Most Active Users of ' + 
          projTitle + ' for ' + timestampHeader, 
      limitResponse);
  } else {
    var emailOfOwnerOfScript = getOwnersOfAllScripts(
      projectFilter, 
      input.projectId);
