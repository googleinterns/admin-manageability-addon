...
function createRule(e) {
  ...
  if (files.hasNext()) {
    ...
    if(input.triggerFrequency == "EVERY_HOUR") {
      trigger.timeBased().EVERY_HOURs(1).create();
    }
    else if(input.triggerFrequency == "EVERY_6_HOUR") {
      trigger.timeBased().EVERY_HOURs(6).create();
    } 
    ...
    var rowContent = ["Rule Type", "Trigger Frequency", "Project Type", "Project Id", "Maximum Limit", "Admin Email"];
    active.appendRow(rowContent);
    var lastRow = active.getLastRow();
    var lastCol = active.getLastColumn();
    active.getRange(lastRow, 1, 1, lastCol).setFontSize(12).setFontWeight("BOLD");
    rowContent = [input.ruleType, input.triggerFrequency, input.ruleProjectFilter, projectId, input.maxLimit, Session.getActiveUser().getEmail()];
    active.appendRow(rowContent);   
    var trigger = ScriptApp.newTrigger("mostExecutedTrigger");
    if(input.triggerFrequency == "EVERY_HOUR") {
      trigger.timeBased().EVERY_HOURs(1).create();
