...
function editRule(e){
  ...
  if (files.hasNext()) {
    ...
    e.formInput = {"ruleType" : ruleData[0], "triggerFrequency" : ruleData[1], "ruleProjectFilter" : ruleData[2], "projectId" : ruleData[3], "maxLimit" : ruleData[4]};
...
function editRuleUI(e){
  ...
  var triggerFrequencySection = CardService.newCardSection().setHeader("<b>TRIGGER FREQUENCY</b>").setCollapsible(true);
  ...
  ruleProjectFilter.setOnChangeAction(CardService.newAction()
                                      .setFunctionName("editruleProjectFilterCallback").setParameters({"Number" : ruleNumber.toString()}));
...
function updateRule(e) {
  ...
  if (files.hasNext()) {
    ...
    var lastCol = active.getLastColumn();
    var rowContent = [
      [input.ruleType, input.triggerFrequency, input.ruleProjectFilter, projectId, input.maxLimit, Session.getActiveUser().getEmail()]
    ];
