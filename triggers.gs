...
function mostExecutedTrigger() {
  ...
  if (files.hasNext()) {
    ...
    if(values[2] != "SPECIFIC_PROJECT") {
      var mostExecutedScript = getMostExecutedScriptFromAllCloudProjects(titleFromTime.fromTime, values[2]);
      for(var i = 0; i<mostExecutedScript.length; i++) {
        if(mostExecutedScript[i].value > values[4]) {
          ...
          body += "Your script has exceeded the allowed limit of executions per "+titleFromTime.title+". This will be reported to the admin.\n";
      ...
      var processes = getNumberOfExecutionOfScript(values[3], titleFromTime.fromTime);
