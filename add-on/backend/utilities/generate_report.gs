 ...
 */
function generateReportForMostExecutedScript(mostExecutedScript,
  timestampHeader, limitResponse) {

  //Create a new date
  var timeStamp = new Date();
  var dateTimeAsString = timeStamp.toString().slice(0, 15);
  var newSheet = SpreadsheetApp.create(timestampHeader + " at " +
    dateTimeAsString);
  ...
  for (var i = 0; i < mostExecutedScript.length && i < limitResponse; i++) {
    ...
    rowContent = [mostExecutedScript[i]["key"], mostExecutedScript[i]["value"],
      mostExecutedScript[i]["GCPId"]];
    active.appendRow(rowContent);
  }
 ...
 * @return {String} URL of generated report
 */
function generateReportForMostActiveUsers(mostActiveUser, timestampHeader,
  limitResponse) {
  ...
  active.appendRow(rowContent);
  var lastRow = active.getLastRow();
  active.getRange(lastRow, 1, 1, active.getLastColumn()).setFontSize(12)
    .setFontWeight("Bold");
 ...
 * @param {Object} emailOfOwnerOfScript is the array of object 
 *    having name and email of owner of appscript
 * @param {String} title is the heading for the report sheet
 * @return {String} URL of generated report
 */
function generateReportForOwnerOfScripts(emailOfOwnerOfScript,
  ...
  for (var i = 0; i < emailOfOwnerOfScript.length; i++) {
    rowContent = [emailOfOwnerOfScript[i].name, emailOfOwnerOfScript[i].email,
      emailOfOwnerOfScript[i].projectId];
    active.appendRow(rowContent);
  }
  return newSheet.getUrl();
}
