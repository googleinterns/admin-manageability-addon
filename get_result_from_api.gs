...
function getResultFromAPIWithToken(e) {
  ...
  if(urlEnd == 'mostExecutedScript') {
    reportUrl = generateReportForMostExecutedScript(result, reportTitle, limitResponse); 
  } else if(urlEnd == 'mostActiveUsers') {
    reportUrl = generateReportForMostActiveUsers(result, reportTitle, limitResponse); 
  } else {
